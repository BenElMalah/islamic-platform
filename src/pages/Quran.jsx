...
export default function Quran() {
  ...
  const getSurahStartPages = () => {
    const starts = {}
    for (const [pageNum, data] of Object.entries(mushafPages)) {
      for (const s of data.surahs) {
        if (!starts[s]) starts[s] = Number(pageNum)
      }
    }
    return starts
  }

  const surahStartPages = getSurahStartPages()
          ...
          {loadingSurahs ? (
            ...
            <ul>
              {filteredSurahs.map((surah) => {
                ...
                const startPage = surahStartPages[surah.number]
                return (
                  <li key={surah.number}>
                    <button
                      onClick={() => startPage && goToPage(startPage)}
                      className={`w-full text-left px-4 py-3 border-b border-gray-100 transition-all ${
                        pageSurahs.includes(surah.number)
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-baseline justify-between gap-2">
                            <p className="font-semibold text-gray-800 text-sm truncate">{info?.en || surah.englishName}</p>
                            <p className="text-sm text-emerald-800 flex-shrink-0" style={{fontFamily:'Amiri,serif'}}>{info?.ar || surah.name}</p>
                          </div>
                          <p className="text-xs text-gray-500 mt-0.5">{info?.tr} &bull; {surah.verseCount} ayahs{startPage ? ` • p.${startPage}` : ''}</p>
                        </div>
                      </div>
                    </button>
                  </li>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-4 shadow-sm">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">Mushaf Page {currentPage} of {TOTAL_PAGES}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-16 text-center text-sm border border-gray-300 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-emerald-500"
            />
          ...
          <div className="w-full max-w-3xl">
            <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden" style={{minHeight:'700px'}}>
              {pageSurahs.length > 0 && pageSurahs.some(s => {
                const pageVerses = pageData.verses.filter(v => v.surah === s)
                return pageVerses[0]?.verse_number === 1
              }) && (
            ...
            <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-b from-emerald-50 to-white">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{surahNames[selectedVerse.surah]?.ar}</p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-700 leading-relaxed text-sm bg-gray-50 p-4 rounded-lg border border-gray-100 italic">
                  &ldquo;{selectedVerse.text_english}&rdquo;
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-600 leading-relaxed text-sm">
                  Tafsir for this verse will be available soon.
                </p>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-gray-100 pt-4">
                <button
                  onClick={() => {
                    ...
                    if (idx > 0) setSelectedVerse(pageData.verses[idx - 1])
                      ...
                      const prevPage = mushafPages[String(currentPage - 1)]
                      if (prevPage?.verses.length) {
                        setCurrentPage(currentPage - 1)
                        setSelectedVerse(prevPage.verses[prevPage.verses.length - 1])
                      }
                    }
                  }}
                ...
                <span className="text-xs text-gray-400 font-medium">Page {selectedVerse.page}</span>
                <button
                  onClick={() => {
                    const idx = pageData.verses.findIndex(v => v.surah === selectedVerse.surah && v.verse_number === selectedVerse.verse_number)
                    if (idx < pageData.verses.length - 1) setSelectedVerse(pageData.verses[idx + 1])
                    else if (currentPage < TOTAL_PAGES) {
                      const nextPage = mushafPages[String(currentPage + 1)]
                      if (nextPage?.verses.length) {
                        setCurrentPage(currentPage + 1)
                        setSelectedVerse(nextPage.verses[0])
                      }
                    }
                  }}
